...
int main(void) {
	...
	status = xTaskCreate(task_led_orange, "led_orange", 200, NULL, 2,
	...
	configASSERT(status == pdPASS);
	status = xTaskCreate(task_led_blue, "led_blue", 200, NULL, 2,
			&taskHandle_led_blue);
...
/*
 *
 * 			enter sleep mode when idle task running to saving power
 *
 * */
